...
# Converts rgb color to a string that can be used to change the text color if supported.
...
def align(*values: str, fill=True, qd=False, override=-1) -> None or str:
    ...
    for i in sorted(sorted_values.keys()):
        ...
        if i != get_constraint():
            ...
            processed = processed+" "*int((i-len(processed))-len(v[1])) + v[1]
    if len(processed) < get_constraint() and fill:
...
def display() -> str:
    if not Border:
        ...
        comp = f"{' '*Padding}{BorderStyle['X'][0]}{comp}{BorderStyle['X'][0]}"
    for i in Memory:
        if not Border:
            ...
            if not isline:
                ...
                white_space = ""
            comp = comp+"\n"+" "*Padding+f"{BorderStyle['X'][0]}{white_space}{i}{white_space}{BorderStyle['X'][0]}"
    if Border:
