...
class Validate:
    ...
    @classmethod
    def date(cls, input_val: str, date_format="%Y-%m-%d", return_type="datetime") -> bool and str or datetime:
        error = False
        sep_chr = date_format[date_format[1:].find("%")]
        input_val = strip(input_val, sep_chr)
        try:
            input_val = datetime.strptime(input_val, date_format)
            if return_type == "string":
                input_val = input_val.strftime(date_format)
        except ValueError:
            comp = ""
            for i in date_format.split(sep_chr):
                if i in Format_Characters:
                    comp = comp + sep_chr + Format_Characters[i]
            input_val = f"Error: {input_val} does not follow format: {comp[1:]}"
            error = True
        return error, input_val
